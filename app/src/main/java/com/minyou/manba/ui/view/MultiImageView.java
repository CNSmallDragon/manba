...
public class MultiImageView extends LinearLayout {
	...
	private int pxOneWandH = CommonUtil.dip2px(getContext(),200);  // 单张图最大允许高度
	private int pxMoreWandH = 0;// 多张图的宽高
	private int pxImagePadding = CommonUtil.dip2px(getContext(), 3);// 图片间的间距

	private int MAX_PER_ROW_COUNT = 3;// 每行显示最大数
	private int max_row_count = 3;		// 每行显示最大数，默认3

	private LayoutParams onePicPara;
	private LayoutParams morePara, moreParaColumnFirst;
	private LayoutParams rowPara;
	private LayoutParams singleColumn;		//每行只显示一列
	...
	public MultiImageView(Context context, AttributeSet attrs) {
		super(context, attrs);
		// 获取属性集
		TypedArray typedArray = context.obtainStyledAttributes(attrs,R.styleable.multi_imageview);
		max_row_count = typedArray.getInteger(R.styleable.multi_imageview_maxRowCount,3);	// 默认为3
		typedArray.recycle();
	}
	...
	private void initImageLayoutParams() {
		...
		onePicPara = new LayoutParams(match, pxOneWandH);

		singleColumn = new LayoutParams(match,wrap);
...
//			}
	...
	private ImageView createImageView(int position, final boolean isMultiImage) {
		...
		if (isMultiImage) {
			if(max_row_count == 1){
				imageView.setLayoutParams(rowPara);
			}else{
